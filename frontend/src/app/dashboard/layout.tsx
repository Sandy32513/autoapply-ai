...
export default function DashboardLayout({
  ...
  return (
    <div className="min-h-screen" style={{ background: 'var(--background)' }}>
      <nav className="glass" style={{ borderBottom: '1px solid var(--card-border)', position: 'sticky', top: 0, zIndex: 50 }}>
          ...
          <div className="flex justify-between h-16">
            <div className="flex items-center">
              <div className="flex-shrink-0 flex items-center gap-2">
                ...
                <Link href="/dashboard" className="text-lg font-semibold" style={{ color: 'var(--foreground)' }}>
                  AutoApply AI
                </Link>
              </div>
              <div className="ml-10 flex items-center gap-1">
                {navItems.map((item) => (
                  <Link
                    ...
                    style={{
                      color: pathname === item.href || (item.href !== '/dashboard' && pathname.startsWith(item.href))
                        ? 'var(--primary)'
                        : 'var(--muted)',
                      background: pathname === item.href || (item.href !== '/dashboard' && pathname.startsWith(item.href))
                        ? 'var(--accent)'
                        : 'transparent',
                    }}
                  >
